...
featureProfile = namedtuple("featureProfile", ["top_contexts", "top_tokens"])
...
def examine_dimension(
    ...
    # )
    return featureProfile(top_contexts, top_tokens)
...
def examine_multiple_dimensions(
    ...
    """
    Returns:
      top_tokens:        list[D][k] of (decoded_token_str, mean_activation_float)
      decoded_contexts:  list[D][k][n] of decoded token strings (left-padded with "")
      activations:       np.ndarray shape [D, k, n] (left-padded with 0.0)
    """
    ...
    return multiFeatureProfile(top_tokens, decoded_contexts, top_activations)
